...
sudo docker exec -it app_server_mysqlsrv_1 bash /var/lib/mysql/mysql_set_native_password.sh

# start mysql service to be able to connect through mysql cli library dependences

sudo docker exec -it app_server_server_app_1 bash service mysql start

# exec git shell script seting up git init and remote to app base repository

sudo docker exec -it app_server_server_app_1 bash /usr/local/apache2/htdocs/app/git_presets.sh 
